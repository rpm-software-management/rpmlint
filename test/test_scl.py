...
from rpmlint.SCLCheck import SCLCheck

from Testing import CONFIG, getTestedPackage, getTestedSpecPackage
...
def test_nonscl_binary_silent(package):
    '''SCL check on non-SCL spec has to be silent'''
    CONFIG.info = True
    output = Filter(CONFIG)
    test = SCLCheck(CONFIG, output)
    test.check(getTestedPackage(package))
    out = output.print_results(output.results)
    assert not out
...
def test_scl_source_rpm(package):
    '''
...
def test_scl_binary_rpm(package):
    '''
...
def test_scl_name_screwed_up(package):
    '''
...
def test_scl_forbidden_folders(package):
...
def test_scl_macros_outside_of_build(package):
    '''SCL check on SCL package that has files in forbidden folders'''
    CONFIG.info = True
    output = Filter(CONFIG)
    test = SCLCheck(CONFIG, output)
