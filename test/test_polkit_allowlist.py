...
def polkit_rules_allowed_check(tmpdir):
    ...
    CONFIG.configuration['PolkitRulesWhitelist'] = ('allowed-polkit-rules.json',)

    output = Filter(CONFIG)
    test = PolkitRulesAllowedCheck(CONFIG, output)

    # make an empty allow list
    with open(os.path.join(tmpdir, 'allowed-polkit-rules.json'), 'w') as fd:
        json.dump({'polkit-rules-test': {'audits': {'bsc#0': {'comment': '', 'digests': {
